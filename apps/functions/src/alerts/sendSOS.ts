...
export interface SmsProvider {
  sendSms: (to: string, message: string) => Promise<{ messageId: string; success: boolean }>;
}
...
export async function writeToDLQ(
  ...
  entry: {
    id: string;
    topic: string;
    type: 'sms_emergency_contact' | 'fcm_push_notification';
    alertId: string;
    userId?: string;
...
export async function processSendSosLogic(
  ...
  const fcmSummary: Array<{ target: string; status: 'dispatched' | 'failed' }> = [];
...
export const sendSOS = onCall(async (request) => {
  if (!request.auth) {
    throw new HttpsError('unauthenticated', 'Caller must be authenticated to invoke sendSOS.');
  }

 ...
 */
export async function processDeleteOwnAccountLogic(
  db: Firestore,
  authAdmin: Auth,
  userId: string,
  userRole = 'passenger'
  ...
  try {
    ...
    if (errCode !== 'auth/user-not-found') {
      ...
      throw new HttpsError('internal', 'Failed to delete authentication credentials.');
    }
  }
...
export const deleteOwnAccount = onCall(
  ...
  async (request) => {
    if (!request.auth || !request.auth.uid) {
      throw new HttpsError('unauthenticated', 'Caller must be authenticated to delete their account.');
    }

    // Explicitly derive UID strictly from request.auth.uid; ignore any payload ID
    const userId = request.auth.uid;
    const userRole = (request.auth.token?.activeRole as string) || 'passenger';
    ...
    try {
      ...
      throw new HttpsError('internal', message || 'Failed to complete account deletion sequence.');
    }
  }
);

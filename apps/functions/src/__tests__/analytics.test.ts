...
describe('Cloud Functions — rebuildSaccoAnalytics & updateDailyAnalytics (CF-004, CF-005 & TEST-002)', () => {
  ...
  describe('rebuildSaccoAnalytics', () => {
    ...
    it('successfully computes and persists SACCO analytics for matching sacco_manager and updates sacco document', async () => {
      ...
      expect(result.safetyScore).toBe(83);
  ...
  describe('updateDailyAnalytics', () => {
    ...
    it('rejects malformed analytics date with invalid-argument', async () => {
      const invalidRequest = {
        data: { dateStr: '16-08-2026' },
        auth: {
          uid: 'authority_1',
          token: { activeRole: 'authority' },
        },
      } as any;

      await expect(updateDailyAnalytics.run(invalidRequest)).rejects.toMatchObject({
        code: 'invalid-argument',
      });
    });

    it('rejects impossible calendar dates with invalid-argument', async () => {
      const invalidRequest = {
        data: { dateStr: '2026-02-30' },
        auth: {
          uid: 'authority_1',
          token: { activeRole: 'authority' },
        },
    ...
    it('converges repeated executions onto the same deterministic analytics document', async () => {
      ...
      mockDbData['trips/t1'] = { id: 't1', startTime: '2026-08-16T08:30:00.000Z' };
      mockDbData['vehicles/veh1'] = { id: 'veh1', riskTier: 'high' };

      const first = await processUpdateDailyAnalyticsLogic(mockDb, '2026-08-16');
      const second = await processUpdateDailyAnalyticsLogic(mockDb, '2026-08-16');

      expect(first.docId).toBe('daily_2026-08-16');
      expect(second.docId).toBe(first.docId);
      expect(Object.keys(mockDbData).filter((key) => key.startsWith('analytics/daily_')).length).toBe(1);
      expect(mockDbData['analytics/daily_2026-08-16'].totalTrips).toBe(1);
    });
  });
});

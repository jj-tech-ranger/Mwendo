  ...
  return (
    <div className="min-h-screen bg-background text-on-background flex flex-col font-body-md theme-admin dark">
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <aside
          className={cn(
            'fixed inset-y-0 left-0 w-72 bg-[#1A2E1A] text-emerald-100 border-r border-emerald-900/40 z-50 flex flex-col justify-between shadow-2xl transition-transform duration-300 ease-in-out lg:hidden select-none overflow-y-auto',
            isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'
          )}
          aria-label={t('admin.shell.mobileNav', 'Mobile Navigation')}
        >
          <div>
            <div className="p-md flex items-center justify-between border-b border-emerald-800/40 bg-[#142414]">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="font-headline-lg-mobile text-sm text-emerald-400 font-bold leading-tight">Mwendo Salama</h1>
              ...
              <button
                onClick={() => setIsMobileMenuOpen(false)}
                aria-label={t('admin.shell.closeMenu', 'Close navigation menu')}
                className="p-1.5 rounded-lg text-emerald-300 hover:bg-emerald-800/40"
              >
                ...
                <div key={group.label} className="space-y-1">
                  ...
                  {group.items.map((item) => (
                    <NavLink
                      ...
                      className={({ isActive }) =>
                        cn(
                          'flex items-center gap-3 px-3 py-2.5 rounded-xl font-label-mono text-xs uppercase tracking-wider transition-all duration-150 min-h-[44px]',
                          isActive
          ...
          <div className="p-md border-t border-emerald-800/40 bg-[#142414] space-y-3">
            ...
            <div className="pt-1 border-t border-emerald-800/30 flex items-center justify-between">
              <LanguageToggle />
              <span className="font-label-mono text-[9px] text-emerald-400/60 uppercase">{t('admin.shell.ops')}</span>
            </div>
          </div>
        </aside>
        ...
        {/* Desktop Sidebar */}
        <aside
          className="hidden lg:flex w-64 bg-[#1A2E1A] text-emerald-100 border-r border-emerald-900/40 flex-col justify-between z-30 select-none overflow-y-auto shrink-0"
          aria-label={t('admin.shell.desktopNav', 'Desktop Navigation')}
        >
          <div>
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-y-auto">
          <header className="h-16 border-b border-outline-variant/20 bg-surface-container-lowest px-3 sm:px-lg flex items-center justify-between sticky top-0 z-20 shadow-xs">
            <div className="flex items-center gap-2 min-w-0">
              <button
                onClick={() => setIsMobileMenuOpen(true)}
                className="lg:hidden p-2 -ml-1 rounded-xl text-on-surface-variant hover:text-on-surface hover:bg-surface-container min-h-[44px] min-w-[44px] flex items-center justify-center"
                aria-label={t('admin.shell.toggleMenu', 'Open navigation menu')}
              >
                <span className="material-symbols-outlined text-2xl">menu</span>
              </button>
              <span className="hidden sm:inline font-label-mono text-xs text-on-surface-variant uppercase tracking-wider truncate">
                {t('admin.shell.platformAdmin')}
              </span>
              <span className="hidden sm:inline text-outline text-xs">/</span>
              <span className="font-headline-lg-mobile text-xs sm:text-sm text-on-surface font-bold truncate">
                {getBreadcrumb()}
              </span>
            </div>
            ...
            <div className="flex items-center gap-1 sm:gap-sm shrink-0">
              ...
              <div className="relative">
                <button
                  ...
                  className="p-2 rounded-xl text-on-surface-variant hover:text-on-surface hover:bg-surface-container relative min-h-[44px] min-w-[44px] flex items-center justify-center"
                ...
                {showNotifications && (
                  <div className="absolute right-0 mt-2 w-72 sm:w-80 bg-surface-container-lowest border border-outline-variant/30 rounded-2xl shadow-xl p-md z-50 text-xs space-y-2">
                    <div className="flex items-center justify-between border-b border-outline-variant/20 pb-2">
                      ...
                      <span className="font-label-mono text-[10px] text-primary">
                        {t('admin.shell.unreadCount', { count: 2 })}
                      </span>
                    </div>
                    <div className="space-y-2 max-h-60 overflow-y-auto">
                      <div className="p-2 rounded-lg bg-surface-container-low border border-outline-variant/20 space-y-1">
                        <p className="font-bold text-on-surface text-[11px]">{t('admin.shell.pendingSaccoVerification')}</p>
                        <p className="text-on-surface-variant text-[10px]">{t('admin.shell.pendingSaccoDesc')}</p>
                        <span className="font-label-mono text-[9px] text-outline">10m ago</span>
                      </div>
                      <div className="p-2 rounded-lg bg-surface-container-low border border-outline-variant/20 space-y-1">
                        <p className="font-bold text-rose-600 text-[11px]">{t('admin.shell.cloudFunctionLatency')}</p>
                        <p className="text-on-surface-variant text-[10px]">{t('admin.shell.cloudFunctionDesc')}</p>
              ...
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-1 sm:gap-2 p-1 rounded-xl hover:bg-surface-container min-h-[44px] min-w-[44px] justify-center"
                  aria-label={t('admin.shell.userMenu', 'User menu')}
                >
                ...
                {showUserMenu && (
                  <div className="absolute right-0 mt-2 w-52 bg-surface-container-lowest border border-outline-variant/30 rounded-2xl shadow-xl p-sm z-50 text-xs space-y-1">
                    <div className="p-2 border-b border-outline-variant/20">
                      ...
                      <p className="font-label-mono text-[10px] text-outline truncate">{user?.email}</p>
                    </div>
                    <button
                      onClick={() => {
          ...
          <main className="p-3 sm:p-lg lg:p-xl max-w-7xl w-full mx-auto flex-1 space-y-lg min-w-0">
            <AnimatedOutlet />
          </main>
        </div>
      </div>

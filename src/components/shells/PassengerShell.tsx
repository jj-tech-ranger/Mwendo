              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  cn(
                    ...
                    isActive ? 'text-on-secondary-container font-bold' : 'text-on-surface-variant hover:text-on-surface'
                  )
                }
              >
                {({ isActive }) => (
                  <>
                    <div className={cn('w-10 h-7 rounded-full flex items-center justify-center transition-colors', isActive && 'bg-secondary-container')}>
                      <span className="material-symbols-outlined text-xl">{item.icon}</span>
                    </div>
                    <span className="text-[10px] font-label-bold tracking-tight mt-0.5">{item.label}</span>
                  </>
                )}
              </NavLink>
            ))}

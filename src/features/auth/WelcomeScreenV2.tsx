...
const RouteIllustration: React.FC = () => (
  <div className="relative h-[430px] overflow-hidden rounded-[32px] bg-primary text-on-primary shadow-2xl sm:h-[500px]">
    <style>{`
      @keyframes mwLandingRoute { from { stroke-dashoffset: 0; } to { stroke-dashoffset: -520; } }
      @keyframes mwLandingPulse { 0%,100% { transform: scale(.75); opacity: .18; } 50% { transform: scale(1.12); opacity: .72; } }
      @keyframes mwLandingOrbit { from { transform: rotate(0deg) translateX(74px) rotate(0deg); } to { transform: rotate(360deg) translateX(74px) rotate(-360deg); } }
      @keyframes mwLandingFloat { 0%,100% { transform: translateY(0); } 50% { transform: translateY(-8px); } }
      @keyframes mwLandingSweep { 0% { transform: translateX(-160%); opacity: 0; } 20%,70% { opacity: .32; } 100% { transform: translateX(160%); opacity: 0; } }
      .mw-landing-route { animation: mwLandingRoute 9s linear infinite; }
      .mw-landing-pulse { transform-box: fill-box; transform-origin: center; animation: mwLandingPulse 3.4s ease-in-out infinite; }
      .mw-landing-orbit { transform-box: fill-box; transform-origin: center; animation: mwLandingOrbit 9s linear infinite; }
      .mw-landing-float { animation: mwLandingFloat 5s ease-in-out infinite; }
      .mw-landing-sweep { animation: mwLandingSweep 6s ease-in-out infinite; }
      @media(prefers-reduced-motion:reduce){.mw-landing-route,.mw-landing-pulse,.mw-landing-orbit,.mw-landing-float,.mw-landing-sweep{animation:none!important}}
    `}</style>
    ...
    <svg viewBox="0 0 620 520" className="absolute inset-0 h-full w-full" fill="none" aria-hidden="true">
      ...
      <g transform="translate(78 112)" className="mw-landing-float">
        ...
        <circle cx="25" cy="31" r="8" fill="#86efac" />
        <text x="44" y="27" fill="white" fontSize="12" fontWeight="700">LIVE SAFETY</text>
        <text x="44" y="44" fill="rgba(255,255,255,.62)" fontSize="9">Journey monitoring</text>
      </g>
      <g transform="translate(378 334)" className="mw-landing-float" style={{ animationDelay: '1.3s' }}>
        <rect width="166" height="62" rx="19" fill="rgba(255,255,255,.105)" stroke="rgba(255,255,255,.08)" />
        <circle cx="25" cy="31" r="8" fill="#fde68a" />
        <text x="44" y="27" fill="white" fontSize="12" fontWeight="700">ROAD ALERT</text>
        <text x="44" y="44" fill="rgba(255,255,255,.62)" fontSize="9">Hazard intelligence</text>
      </g>
      <rect x="0" y="0" width="150" height="520" fill="white" opacity=".08" className="mw-landing-sweep" />
    </svg>
    <div className="absolute bottom-6 left-6 right-6 flex items-end justify-between gap-4">
      <div>
        <p className="text-[10px] font-bold uppercase tracking-[.22em] text-white/55">Mwendo Salama</p>
        <p className="mt-1 text-sm font-semibold">A safer view of every journey.</p>
      </div>
      <div className="hidden rounded-2xl border border-white/10 bg-white/10 px-3 py-2 backdrop-blur sm:block">
        <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider text-white/75"><span className="h-2 w-2 rounded-full bg-emerald-300" /> Network active</div>
      </div>
    </div>
  </div>
);
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-surface text-on-surface">
      <header className="sticky top-0 z-50 border-b border-outline-variant/20 bg-surface/85 backdrop-blur-xl">
        <div className="mx-auto flex h-[72px] max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <div className="hidden items-center gap-2 md:flex">
            ...
            <Button variant="outline" size="sm" onClick={() => navigate('/auth/login')}>{t('welcome.nav.logIn')}</Button>
            <Button variant="primary" size="sm" onClick={() => navigate('/location-permission')}>{t('welcome.nav.getStarted')}</Button>
          </div>
          <div className="flex items-center gap-2 md:hidden">
            <LanguageToggle />
            <button onClick={toggleDarkMode} aria-label="Toggle theme" className="flex h-9 w-9 items-center justify-center rounded-xl border border-outline-variant/40"><span className="material-symbols-outlined text-lg">{mode === 'dark' ? 'light_mode' : 'dark_mode'}</span></button>
            <button onClick={() => setMenuOpen((open) => !open)} aria-label="Toggle navigation" className="flex h-9 w-9 items-center justify-center rounded-xl border border-outline-variant/40"><span className="material-symbols-outlined text-xl">{menuOpen ? 'close' : 'menu'}</span></button>
          </div>
        </div>
      ...
      <main>
        <section className="relative overflow-hidden border-b border-outline-variant/20">
          ...
          <div className="relative mx-auto grid max-w-7xl items-center gap-12 px-4 pb-16 pt-12 sm:px-6 sm:pt-16 lg:grid-cols-[1.02fr_.98fr] lg:gap-16 lg:px-8 lg:pb-24 lg:pt-20">
            <div className="max-w-2xl">
              <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-primary/20 bg-primary/10 px-3.5 py-2 text-[10px] font-bold uppercase tracking-[.16em] text-primary"><span className="material-symbols-outlined text-base">shield</span>{t('welcome.hero.badge')}</div>
              <h1 className="text-4xl font-extrabold leading-[1.05] tracking-[-.035em] sm:text-5xl lg:text-6xl">{t('welcome.hero.title')} <span className="text-primary">Mwendo Salama</span>.</h1>
              <p className="mt-6 max-w-xl text-base leading-7 text-on-surface-variant sm:text-lg">{t('welcome.hero.description')}</p>
              <div className="mt-8 flex flex-wrap gap-3">
                <Button variant="primary" size="lg" onClick={() => navigate('/location-permission')} className="rounded-2xl px-6 shadow-lg shadow-primary/15"><span>{t('welcome.hero.startSafeTrip')}</span><span className="material-symbols-outlined ml-1 text-lg">arrow_forward</span></Button>
        ...
        <section id="how-it-works" className="scroll-mt-20 border-b border-outline-variant/20 bg-surface-container-lowest py-20 sm:py-24">
        ...
        <section id="safety" className="scroll-mt-20 border-b border-outline-variant/20 bg-surface py-20 sm:py-24">
          <div className="mx-auto grid max-w-7xl items-center gap-12 px-4 sm:px-6 lg:grid-cols-2 lg:px-8">
            <div className="relative overflow-hidden rounded-[32px] bg-surface-container-low p-7 sm:p-10">
              <div className="absolute right-0 top-0 h-48 w-48 rounded-full bg-primary/10 blur-3xl" />
              <div className="relative"><div className="flex items-center justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[.18em] text-primary">Passenger safety</p><h3 className="mt-2 text-2xl font-extrabold">See the journey as it happens.</h3></div><ShieldSpeedometerMark className="h-12 w-12" /></div><div className="mt-8 rounded-3xl border border-outline-variant/25 bg-surface p-5 shadow-lg"><div className="flex items-center justify-between text-xs font-bold"><span className="text-on-surface-variant">Live speed</span><span className="text-primary">Within limit</span></div><div className="mt-5 flex items-end justify-between"><div><span className="text-5xl font-extrabold tracking-tight">72</span><span className="ml-2 text-xs text-on-surface-variant">km/h</span></div><div className="text-right text-[10px] text-on-surface-variant">SPEED LIMIT<br/><strong className="text-on-surface">80 km/h</strong></div></div><div className="mt-5 h-2 overflow-hidden rounded-full bg-surface-container-high"><div className="h-full w-[85%] rounded-full bg-primary" /></div><div className="mt-5 flex items-start gap-3 rounded-2xl bg-amber-500/10 p-3.5"><span className="material-symbols-outlined text-amber-600">warning</span><div><p className="text-xs font-bold">Road alert nearby</p><p className="mt-1 text-[11px] text-on-surface-variant">Stay aware and review reported hazards ahead.</p></div></div></div></div>
            </div>
            <div className="max-w-xl"><p className="text-[11px] font-bold uppercase tracking-[.2em] text-primary">Safety, not noise</p><h2 className="mt-3 text-3xl font-extrabold tracking-tight sm:text-4xl">Know what is happening on the road.</h2><p className="mt-5 text-sm leading-7 text-on-surface-variant sm:text-base">Mwendo Salama turns a normal PSV trip into a clearer safety experience—with live speed awareness, road-risk visibility, hazard reporting and emergency support.</p><div className="mt-8 grid gap-3 sm:grid-cols-2"><div className="rounded-2xl border border-outline-variant/20 p-4"><span className="material-symbols-outlined text-primary">speed</span><p className="mt-3 text-sm font-bold">Live speed awareness</p></div><div className="rounded-2xl border border-outline-variant/20 p-4"><span className="material-symbols-outlined text-primary">warning</span><p className="mt-3 text-sm font-bold">Community hazard reports</p></div><div className="rounded-2xl border border-outline-variant/20 p-4"><span className="material-symbols-outlined text-primary">emergency</span><p className="mt-3 text-sm font-bold">Emergency support</p></div><div className="rounded-2xl border border-outline-variant/20 p-4"><span className="material-symbols-outlined text-primary">analytics</span><p className="mt-3 text-sm font-bold">Trip safety insights</p></div></div></div>
          </div>
        </section>

        <section id="saccos" className="scroll-mt-20 border-b border-outline-variant/20 bg-surface-container-lowest py-20 sm:py-24">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"><div className="rounded-[36px] bg-primary p-7 text-on-primary shadow-2xl sm:p-10 lg:p-14"><div className="grid items-center gap-10 lg:grid-cols-[1.1fr_.9fr]"><div><p className="text-[10px] font-bold uppercase tracking-[.2em] text-on-primary/60">For SACCOs & safety teams</p><h2 className="mt-3 max-w-2xl text-3xl font-extrabold tracking-tight sm:text-4xl">Turn passenger journeys into actionable safety intelligence.</h2><p className="mt-5 max-w-xl text-sm leading-7 text-on-primary/70">Give fleet teams a clearer view of trips, violations, hazards and safety trends—without losing sight of the people travelling in those vehicles.</p><div className="mt-7"><Button variant="outline" onClick={() => navigate('/auth/login')} className="border-white/25 bg-white/10 text-white hover:bg-white/15">{t('welcome.nav.logIn')} <span className="material-symbols-outlined ml-1">arrow_forward</span></Button></div></div><div className="grid grid-cols-2 gap-3"><div className="rounded-3xl border border-white/10 bg-white/10 p-5"><span className="text-3xl font-extrabold">Live</span><p className="mt-2 text-xs text-on-primary/65">trip visibility</p></div><div className="rounded-3xl border border-white/10 bg-white/10 p-5"><span className="text-3xl font-extrabold">Risk</span><p className="mt-2 text-xs text-on-primary/65">trend awareness</p></div><div className="rounded-3xl border border-white/10 bg-white/10 p-5"><span className="text-3xl font-extrabold">Data</span><p className="mt-2 text-xs text-on-primary/65">ground truth</p></div><div className="rounded-3xl border border-white/10 bg-white/10 p-5"><span className="text-3xl font-extrabold">Kenya</span><p className="mt-2 text-xs text-on-primary/65">built for local roads</p></div></div></div></div></div>
        </section>
